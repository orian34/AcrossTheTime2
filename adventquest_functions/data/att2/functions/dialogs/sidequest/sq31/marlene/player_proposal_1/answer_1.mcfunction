#################################################################
#Made by Adventquest											#
#Use function to process the green answer 						#
#################################################################


#FRENCH LANGUAGE

tellraw @s[scores={LANGUAGE=0}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"De l'aide ? Je veux bien. Ici les hivers sont rudes et nous dépendons beaucoup de la vallée en matière de denrées. Tout le monde compte sur Raphaël, notre convoyeur principal, mais hé ! Y'a pas d'histoires sans péripétie ! On n'a plus eu de nouvelles de lui depuis une semaine. Ça lui arrive d'être en retard, mais là c'est inquiétant, faut qu'on mange ici !","color":"dark_aqua"}]}


#ENGLISH LANGUAGE

tellraw @s[scores={LANGUAGE=1}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"You come along at just the right time. The winters here are harsh and we are very dependent on the valley for food. Everyone is counting on Raphael, our main conveyor, but hey! There are no stories without an incident! We haven't heard from him in a week. Sometimes he's late, but this is worrying, we have to eat here!","color":"dark_aqua"}]}


#CHINESE LANGUAGE

tellraw @s[scores={LANGUAGE=2}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"你來的正是時候。這裡的冬天很嚴酷， 我們非常依賴山谷作為食物。每個人都在指望我們的主要輸送機 Raphael， 但是， 嘿！ 沒有沒有事件的故事！ 我們已經一周沒有他的消息了。有時他會遲到， 但這很令人擔憂， 我們必須在這裡吃飯！ ","color":"dark_aqua"}]}


#JAPANESE LANGUAGE

tellraw @s[scores={LANGUAGE=3}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"助けが必要ですか？ 喜んでお手伝いします。ここでは冬が厳しく、食料は谷に大きく依存しています。皆がメインの運搬者であるラファエルに頼っていますが、まあ！ 事件なしの物語はありません！ 彼からの連絡は一週間ありません。遅れることもありますが、これは心配です。ここで食事をしなければなりません！","color":"dark_aqua"}]}

#KOREAN LANGUAGE

tellraw @s[scores={LANGUAGE=4}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"도움이 필요하신가요? 기꺼이 도와드리겠습니다. 여기의 겨울은 매우 혹독하고, 우리는 식량을 위해 계곡에 많이 의존하고 있습니다. 모두가 우리의 주요 운반자인 라파엘에게 의존하고 있지만, 자! 사건 없는 이야기는 없습니다! 그로부터 소식을 들은 지 일주일이 지났습니다. 때때로 그는 늦기도 하지만, 이번에는 걱정됩니다. 여기서 먹어야 합니다!","color":"dark_aqua"}]}

#ARABIC LANGUAGE

tellraw @s[scores={LANGUAGE=5}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"هل تحتاج إلى مساعدة؟ أنا على استعداد للمساعدة. الشتاء هنا قاسي للغاية ونحن نعتمد بشكل كبير على الوادي في توفير الغذاء. الجميع يعتمد على رافائيل، ناقلنا الرئيسي، لكن هناك! لا توجد قصص بدون حادث! لم نسمع عنه منذ أسبوع. أحياناً يتأخر، لكن هذا مقلق، يجب أن نأكل هنا!","color":"dark_aqua"}]}

#RUSSIAN LANGUAGE

tellraw @s[scores={LANGUAGE=6}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"Нужна помощь? Я с удовольствием помогу. Зимы здесь суровые, и мы сильно зависим от долины в плане продовольствия. Все полагаются на Рафаэля, нашего главного перевозчика, но, эй! Нет событий без происшествий! Мы не слышали о нем уже неделю. Иногда он опаздывает, но это беспокоит, нам нужно есть здесь!","color":"dark_aqua"}]}

#SPANISH LANGUAGE

tellraw @s[scores={LANGUAGE=7}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"¿Necesitas ayuda? Estoy encantada de ayudarte. Los inviernos aquí son duros y dependemos mucho del valle para obtener alimentos. Todos cuentan con Raphael, nuestro principal transportista, ¡pero, eh! ¡No hay historias sin incidentes! No hemos tenido noticias de él durante una semana. A veces se retrasa, pero esto es preocupante, ¡tenemos que comer aquí!","color":"dark_aqua"}]}

#GERMAN LANGUAGE

tellraw @s[scores={LANGUAGE=8}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"Brauchen Sie Hilfe? Ich helfe gerne. Die Winter hier sind hart, und wir sind stark auf das Tal für Nahrung angewiesen. Jeder zählt auf Raphael, unseren Haupttransporter, aber hey! Es gibt keine Geschichten ohne Zwischenfälle! Wir haben seit einer Woche nichts von ihm gehört. Manchmal ist er spät, aber das ist besorgniserregend, wir müssen hier essen!","color":"dark_aqua"}]}

#HINDI LANGUAGE

tellraw @s[scores={LANGUAGE=9}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"मदद चाहिए? मैं खुशी से मदद करूंगी। यहाँ के सर्दी कठोर हैं और हम भोजन के लिए घाटी पर बहुत निर्भर करते हैं। सभी राफेल पर निर्भर हैं, हमारे मुख्य परिवाहक, लेकिन सुनिए! बिना घटना के कोई कहानी नहीं होती! हमें एक हफ्ते से उसकी कोई खबर नहीं मिली। कभी-कभी वह देर से आता है, लेकिन यह चिंताजनक है, हमें यहाँ खाना पड़ता है!","color":"dark_aqua"}]}

#PORTUGUESE LANGUAGE

tellraw @s[scores={LANGUAGE=10}] {"text":" °-° Marlène : ","color":"green","extra":[{"text":"Precisa de ajuda? Eu ficarei feliz em ajudar. Os invernos aqui são rigorosos e dependemos muito do vale para obter alimentos. Todos estão contando com Raphael, nosso principal transportador, mas, ei! Não há histórias sem incidentes! Não tivemos notícias dele há uma semana. Às vezes ele se atrasa, mas isso é preocupante, precisamos comer aqui!","color":"dark_aqua"}]}
