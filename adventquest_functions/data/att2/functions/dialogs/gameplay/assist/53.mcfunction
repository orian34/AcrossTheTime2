...
#FRENCH LANGUAGE
tellraw @s[scores={LANGUAGE=0}] {"text":"⊰☼⊱ ","color":"dark_green","bold":true,"extra":[{"text":"L'épée mythique <Fortuity> fait apparaitre des orbes d'expérience supplémentaires pour chaque coup porté à un ennemi. Elle est la seule arme obtenue au début du jeu ou peut être obtenue dans les maisons de héros.","color":"gray","bold":false}]}
