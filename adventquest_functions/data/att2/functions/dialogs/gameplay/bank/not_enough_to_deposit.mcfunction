#################################################################
#Made by Adventquest											#
#Deposit from bank to personal stock							#
#################################################################


#FRENCH LANGUAGE

tellraw @s[scores={LANGUAGE=0}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"Désolé, mais vous n'avez pas assez de chronoton sur vous pour en déposer autant...","color":"gold"}]}


#ENGLISH LANGUAGE

tellraw @s[scores={LANGUAGE=1}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"Sorry, but you don't have enough chronoton to deposit that amount...","color":"gold"}]}


#CHINESE LANGUAGE

tellraw @s[scores={LANGUAGE=2}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"抱歉， 現有餘額小於待存入餘額。","color":"gold"}]}


#JAPANESE LANGUAGE
tellraw @s[scores={LANGUAGE=3}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"申し訳ありませんが、その額を預けるのに十分なクロノトンがありません...","color":"gold"}]}

#KOREAN LANGUAGE
tellraw @s[scores={LANGUAGE=4}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"죄송합니다만, 해당 금액을 입금할 크로노톤이 충분하지 않습니다...","color":"gold"}]}

#ARABIC LANGUAGE
tellraw @s[scores={LANGUAGE=5}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"عذرًا، لكن ليس لديك ما يكفي من الكرونوتون لإيداع هذا المبلغ...","color":"gold"}]}

#RUSSIAN LANGUAGE
tellraw @s[scores={LANGUAGE=6}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"Извините, но у вас недостаточно хронотонов для внесения этой суммы...","color":"gold"}]}

#SPANISH LANGUAGE
tellraw @s[scores={LANGUAGE=7}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"Lo siento, pero no tienes suficientes cronotones para depositar esa cantidad...","color":"gold"}]}

#GERMAN LANGUAGE
tellraw @s[scores={LANGUAGE=8}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"Entschuldigung, aber du hast nicht genug Chronoton, um diesen Betrag einzuzahlen...","color":"gold"}]}

#HINDI LANGUAGE
tellraw @s[scores={LANGUAGE=9}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"क्षमा करें, लेकिन आपके पास इस राशि को जमा करने के लिए पर्याप्त क्रोनोटोन नहीं है...","color":"gold"}]}

#PORTUGUESE LANGUAGE
tellraw @s[scores={LANGUAGE=10}] {"text":" °-° Gaelle : ","color":"green","extra":[{"text":"Desculpe, mas você não tem chronoton suficiente para depositar essa quantidade...","color":"gold"}]}
