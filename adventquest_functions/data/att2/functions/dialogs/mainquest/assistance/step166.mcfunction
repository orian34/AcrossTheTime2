...
tellraw @s[scores={LANGUAGE=0}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"L'émeraude ne peut pas bouger, j'ai besoin d'activer un mécanisme pour continuer. A côté doit se trouver un autre système qui débloquera la situation...","color":"aqua"}]}
