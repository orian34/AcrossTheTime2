...
tellraw @s[scores={LANGUAGE=0}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"Il faut que je trouve trois fragments d'échelles afin de grimper sur la barricade, passer par-dessus la végétation et trouver une entrée vers le temple.","color":"aqua"}]}

#ENGLISH LANGUAGE

tellraw @s[scores={LANGUAGE=1}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"I have to find three fragments of ladders in order to climb the barricade, pass over the vegetation and find an entrance to the temple.","color":"aqua"}]}
