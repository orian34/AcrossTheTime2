...
tellraw @s[scores={LANGUAGE=1}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"Here I am inside a large structure, it must hide inside it a Neleptron. Let's find a way to go down by activating these mechanisms.","color":"aqua"}]}

#CHINESE LANGUAGE

tellraw @s[scores={LANGUAGE=2}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"我在一個大結構裡面， 裡面一定藏著一個 Neleptron。讓我們通過激活這些機制找到一種方法。","color":"aqua"}]}


# JAPANESE LANGUAGE

tellraw @s[scores={LANGUAGE=3}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"ここには大きな構造物の中にいます。中には必ずや Neleptron が隠されているはずです。これらのメカニズムを活性化して下降する方法を見つけましょう。","color":"aqua"}]}

# KOREAN LANGUAGE

tellraw @s[scores={LANGUAGE=4}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"여기 큰 구조물 안에 있습니다. 그 안에 Neleptron 이 숨겨져 있을 것입니다. 이 메커니즘을 활성화하여 내려가는 방법을 찾아보겠습니다.","color":"aqua"}]}
...
tellraw @s[scores={LANGUAGE=8}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"Hier bin ich in einer großen Struktur, sie muss drinnen einen Neleptron verstecken. Finden wir einen Weg nach unten, indem wir diese Mechanismen aktivieren.","color":"aqua"}]}
