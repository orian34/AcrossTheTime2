...
execute if score Real1 TIMER matches 1 run data merge entity 00000000-0000-089a-0000-00000000089a {Invulnerable:0,Attributes:[{Name:generic.max_health,Base:10.0}],Health:10,AbsorptionAmount:0,HandDropChances:[-2.0F,1.0F],ArmorDropChances:[-2.0F,-2.0F,-2.0F,-2.0F],HandItems:[{},{id:"minecraft:nether_star",Count:1b,tag:{EquipmentType:"que",Rarity:"que",display:{Name:"{\"text\":\"§cMédaillon\"}","Lore":["{\"text\":\"§4§oMedaillon\"}"]}}}]}
...
execute if score Real1 TIMER matches 700 run kill @e[type=minecraft:item,nbt={display:{"Lore":["{\"text\":\"§4§oMedaillon\"}"]}}]
execute if score Real1 TIMER matches 700 run clear @a minecraft:nether_star{display:{"Lore":["{\"text\":\"§4§oMedaillon\"}"]}}
