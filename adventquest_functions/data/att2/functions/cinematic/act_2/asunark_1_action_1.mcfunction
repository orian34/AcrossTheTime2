##################################################
#Made by Adventquest                             #
#Process asunark_1_action_1		 				 #
##################################################

function att2:dialogs/title/a2_ch7_title1
function att2:dialogs/title/a2_ch7_subtitle1
function att2:cinematic/act_2/asunark/initialize
function att2:cinematic/act_2/asunark/wing_e/initialize
function att2:cinematic/act_2/asunark/wing_n/initialize
function att2:cinematic/act_2/asunark/wing_s/initialize
function att2:cinematic/act_2/asunark/center/initialize
function att2:cinematic/act_2/asunark/outside/initialize