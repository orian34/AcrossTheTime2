...
execute anchored eyes at @s positioned ^ ^ ^1.0 run summon minecraft:fireball ~ ~ ~ {Tags:["FireballSpell","STOP"],ExplosionPower:1,Motion:[0.0,0.0,0.0]}
...
execute as @e[tag=STOP,type=fireball] store result entity @s Motion[0] double 0.015 run scoreboard players get @s MOTIONX
execute as @e[tag=STOP,type=fireball] store result entity @s Motion[1] double 0.015 run scoreboard players get @s MOTIONY
execute as @e[tag=STOP,type=fireball] store result entity @s Motion[2] double 0.015 run scoreboard players get @s MOTIONZ
