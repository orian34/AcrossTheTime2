...
execute if entity @s[scores={LANGUAGE=2}] run data merge entity @e[tag=NewPet,type=minecraft:bat,distance=0..5,limit=1,sort=nearest] {CustomName:"\"治愈之友\""}

scoreboard players operation @e[tag=NewPet] FOLLOW_PLAYER = @s NUMEROJOUEUR
execute as @e[tag=NewPet] run tag @s add VitalityPet2
