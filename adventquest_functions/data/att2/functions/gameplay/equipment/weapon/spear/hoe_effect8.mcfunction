...
scoreboard players set @s MOTIONY 29
scoreboard players operation @s MOTIONZ -= @a[scores={HOEDAMAGE=1..},limit=1] MOTIONZ
execute store result entity @s Motion[0] double 0.012 run scoreboard players get @s MOTIONX
execute store result entity @s Motion[1] double 0.02 run scoreboard players get @s MOTIONY
execute store result entity @s Motion[2] double 0.012 run scoreboard players get @s MOTIONZ
##make range damage
$damage @s $(spear) minecraft:player_attack by @a[scores={HOEDAMAGE=1..},limit=1]
#Particle
particle minecraft:enchanted_hit ~ ~0.5 ~ 0.2 0.25 0.2 0.4 31 normal
particle minecraft:crit ~ ~0.5 ~ 0.3 0.25 0.3 0.4 31 normal