...
execute as @a at @s positioned ^ ^ ^2.5 as @e[team=hostile,distance=..2,scores={GAMELEVEL=0..}] at @s store result bossbar healthbar max run data get entity @s Attributes[{Name:"minecraft:generic.max_health"}].Base
