##################################################
#Made by Adventquest                             #
#Initialize health regeneration                  #
##################################################

scoreboard objectives add HER_TOT dummy
scoreboard players set @s HER_TOT 0

scoreboard objectives add HER_BASE dummy
scoreboard players set @s HER_BASE 0

scoreboard objectives add HER_LVL_LE dummy
scoreboard players set @s HER_LVL_LE 0

scoreboard objectives add HER_LVL_OH dummy
scoreboard players set @s HER_LVL_OH 0

scoreboard objectives add HER_LVL_PO dummy
scoreboard players set @s HER_LVL_PO 0

scoreboard objectives add HER_LVL_DAB dummy
scoreboard players set @s HER_LVL_DAB 0

scoreboard objectives add HER_LVL_DAM dummy
scoreboard players set @s HER_LVL_DAM 0

scoreboard objectives add HER_LVL_EQ1 dummy
scoreboard players set @s HER_LVL_EQ1 0

scoreboard objectives add HER_LVL_EQ2 dummy
scoreboard players set @s HER_LVL_EQ2 0

scoreboard objectives add HER_LVL_EQ3 dummy
scoreboard players set @s HER_LVL_EQ3 0

scoreboard objectives add HER_LVL_EQ4 dummy
scoreboard players set @s HER_LVL_EQ4 0

scoreboard objectives add HER_LVL_SET dummy
scoreboard players set @s HER_LVL_SET 0

scoreboard objectives add HER_LVL_EXT dummy
scoreboard players set @s HER_LVL_EXT 0

scoreboard objectives add HER_FOOD food

scoreboard objectives add TIMER_HER_TOT dummy
scoreboard players set @s TIMER_HER_TOT 10
scoreboard objectives add TIMER_HER_LE dummy
scoreboard objectives add TIMER_HER_OH dummy
scoreboard objectives add TIMER_HER_PO dummy
scoreboard objectives add TIMER_HER_DAB dummy
scoreboard objectives add TIMER_HER_DAM dummy
scoreboard objectives add TIMER_HER_EQ1 dummy
scoreboard objectives add TIMER_HER_EQ2 dummy
scoreboard objectives add TIMER_HER_EQ3 dummy
scoreboard objectives add TIMER_HER_EQ4 dummy
scoreboard objectives add TIMER_HER_SET dummy
scoreboard objectives add TIMER_HER_EXT dummy

scoreboard objectives add HER_UPGRADE_REQ dummy
scoreboard players set @s HER_UPGRADE_REQ 1